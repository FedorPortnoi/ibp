"""
Cycle 3 Test - OK People Search Verification
=============================================
Tests the OK People Search (real name search, not username guessing).
"""
...
from app.services.phase1.ok_people_search import OKPeopleSearch, search_ok_people


def test_ok_people_search_class():
    """Test OKPeopleSearch class initialization and methods."""
    print("=" * 60)
    print("TEST: OKPeopleSearch Class")
    print("=" * 60)
    ...
    variations2 = searcher.generate_search_variations("Ангелина Пилюшина")
    print(f"\n   Input: 'Ангелина Пилюшина'")
    print(f"   Variations: {variations2}")
    has_diminutive = any('геля' in v.lower() or 'лина' in v.lower() for v in variations2)
    if has_diminutive:
...
def test_ok_people_search_live():
    """Test actual OK people search (requires network)."""
    print("\n" + "=" * 60)
    print("TEST: Live OK People Search")
    print("=" * 60)
    print("NOTE: This test requires network access and may require login")

    try:
        # Test with a common Russian name
        results = search_ok_people("Иван Иванов", limit=5)
        ...
        if results:
            ...
            print("   [WARN] No profiles found (OK may require login)")

        # Test with target name for similarity
        results2 = search_ok_people("Тихон Портной", limit=5, target_name="Тихон Портной")
...
def test_integration_with_combined_search():
    """Test that OK People Search integrates with combined search."""
    print("\n" + "=" * 60)
    ...
    try:
        ...
        print("   [PASS] CombinedSearchService imports OKPeopleSearch")

        # Check that ok_people_search is imported in the module
        import app.services.combined_search as cs
        has_ok_people = hasattr(cs, 'ok_people_search')
        if has_ok_people:
            print("   [PASS] ok_people_search is available in combined_search module")
        else:
            print("   [FAIL] ok_people_search not found in combined_search")
            return False

        # Check the docstring includes OK People Search
        if 'OK People Search' in cs.CombinedSearchService.__doc__:
            print("   [PASS] OK People Search documented in pipeline")
        else:
            print("   [WARN] OK People Search not in docstring")
...
def main():
    print("\n" + "=" * 60)
    print("      CYCLE 3 TEST SUITE - OK People Search")
    print("=" * 60)

    tests = [
        ("OKPeopleSearch Class", test_ok_people_search_class),
        ("Live OK Search", test_ok_people_search_live),
    ...
    print("\n" + "=" * 60)
    if all_passed:
        print("   ALL TESTS PASSED - Cycle 3 OK People Search is working!")
    else:
