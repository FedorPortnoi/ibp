...
def dossier_page(check_id):
    ...
    from app.services.candidate.connection_graph import build_from_check, build_graph_data
    _connections = build_from_check(check)
    _graph_data = _json.dumps(build_graph_data(_connections, check))
    ...
    return render_template(
        ...
        court_source_statuses=check.court_source_statuses or {},
        fssp_records=check.fssp_records,
        fssp_status=check.fssp_status or '',
        source_statuses=check.source_statuses or {},
        ...
        connection_graph={'confirmed': [c.to_dict() for c in _connections if c.confidence == 'strong'],
                          'possible':  [c.to_dict() for c in _connections if c.confidence != 'strong']},
        connection_graph_json=_graph_data,
    )
...
def graph_page(check_id):
    ...
    from app.services.candidate.connection_graph import build_from_check, build_graph_data
    _connections = build_from_check(check)
    connection_graph_json = _json.dumps(build_graph_data(_connections, check))
