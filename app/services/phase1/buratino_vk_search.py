...
class BuratinoVKSearch:
    ...
    def lookup_by_phone(self, phones: list) -> List[VKProfileResult]:
        """
        Find VK accounts registered to the given phone numbers using
        account.lookupContacts (requires VK_USER_TOKEN with friends scope).

        This finds burner/secondary accounts that don't appear in name search
        because every VK account must be registered with a phone number.

        Returns list of VKProfileResult (may include accounts with no photo/name).
        """
        user_token = os.environ.get('VK_USER_TOKEN', '').strip()
        if not user_token or not phones:
            return []

        # Normalize to E.164 format — VK expects +7XXXXXXXXXX
        import re as _re
        normalized = []
        for p in phones:
            digits = _re.sub(r'\D', '', str(p))
            if len(digits) == 11 and digits[0] in ('7', '8'):
                normalized.append('+7' + digits[1:])
            elif len(digits) == 10:
        ...
        try:
            import json
            params = {
                'contacts': json.dumps(normalized),
                'service': 'phone',
                'fields': ','.join(self.PROFILE_FIELDS),
                'access_token': user_token,
                'v': self.API_VERSION,
            ...
            self._rate_limit()
            resp = self.session.post(
                f'{self.API_BASE_URL}/account.lookupContacts',
                data=params,
                timeout=15,
            )
            data = resp.json()
            if 'error' in data:
                logger.warning(
                    'VK lookupContacts error %s: %s',
                    data['error'].get('error_code'),
                    data['error'].get('error_msg'),
                )
                return []

            found = (data.get('response') or {}).get('found') or []
            results = []
            for item in found:
                ...
                profile = VKProfileResult(
                    ...
                    screen_name=item.get('screen_name') or item.get('domain'),
                    photo_url=(
                        item.get('photo_max_orig') or item.get('photo_400_orig')
                        or item.get('photo_200') or item.get('photo_100')
                    ),
                    city=(item.get('city') or {}).get('title'),
                    birth_date=item.get('bdate'),
                    age=self._calculate_age(item.get('bdate')),
                ...
                logger.info(
                    'VK phone lookup: found id%s %s %s (phone match)',
                    uid, item.get('first_name', ''), item.get('last_name', ''),
                )

            logger.info(
                'VK lookupContacts: %d phones → %d accounts found',
                len(normalized), len(results),
            )
            return results

        except Exception as exc:
            logger.warning('VK lookupContacts failed: %s', exc)
            return []
